...
data_country <- aggregate(as.numeric(contact_lgbti) ~ isocntry, data = final_data, mean, na.rm = TRUE)
...
mapCountryData(map_data, 
               ...
               catMethod = "quantiles", 
               colourPalette = "Blues", #puedo poner otra paleta si no os gusta(colourPalette = brewer.pal(9, "Blues")))
               mapRegion = "Europe")  
...
imputed_rf = mice(mice_data, method = "rf", 
                  m = 3, maxit = 3, seed = 123)

imputed_pmm = mice(mice_data, method = "pmm",
                   m = 3, maxit = 3, seed = 123)
...
imputed_comparison <- data.frame(
  ...
  imputed_rf = complete(imputed_rf)$ideology,
  imputed_pmm = complete(imputed_pmm)$ideology)
...
m <- plot <- ggplot(data_imputed_long, aes(x = value, fill = title)) +
